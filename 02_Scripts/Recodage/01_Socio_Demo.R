...
DB %<>%
  ...
  mutate(POND = poids_cal/(sum(poids_cal)/nrow(.)), #Pour l'ACM et les tableaux : le poids somme à la taille de l'échantillon
...
DB %<>% mutate(
  ...
  CSP_3 = 
    factor(CS_E_NIV3,
           ...
           labels = 
             c("Agriculteurs petite exploitation",
  ...
  religion = 
    factor(
      REL1E,
      levels = c("00", "01", "02", "03", "04", "05", "06", "88", "99"),
      labels = c(
  ...
  religion_conjoint = 
    factor(
      REL1C,
      levels = c("00", "01", "02", "03", "04", "05", "06", "88"),
      labels = c(
        "Sans religion",
        "Catholique (y compris syriaque, chaldéenne, maronite)",
        "Musulmane",
